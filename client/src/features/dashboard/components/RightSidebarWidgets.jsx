...
export default function RightSidebarWidgets({
  ...
  const upcoming = (upcomingAuctions || []).map((item) => ({
    id: item.id,
    title: item.title,
  ...
  const notifications = (activity || []).slice(0, 4).map((item, index) => ({
    id: item.id || `activity-${index}`,
    title: item.title || item.type || 'Activity',
    body: item.description || item.message || 'Recent marketplace activity',
    time: item.time || item.createdAtLabel || 'Recently',
  }))
    ...
    <aside className="space-y-5" aria-label="Dashboard widgets">
          ...
          <ul className="space-y-3">
            {upcoming.map((item) => (
              <li key={item.id}>
                <Link
                  to={`/auctions/${item.id}`}
                  className="flex items-center gap-3 rounded-lg transition-colors hover:bg-neutral-50"
                >
                  {item.image ? (
                    <img
                      src={item.image}
                    ...
                    <div className="h-11 w-11 rounded-lg bg-neutral-100 ring-1 ring-border/70" />
                  )}
        ...
        {notifications.length ? (
          <ul className="space-y-3">
            {notifications.map((item) => (
              <li
                key={item.id}
                className="rounded-lg border border-border/60 bg-neutral-50 px-3 py-2.5"
              >
      ...
      <section className="rounded-xl border border-border/70 bg-white p-5 shadow-sm">
        ...
        <div className="flex items-center justify-between gap-2">
          <p className="text-sm text-muted-foreground">Marketplace services are online.</p>
        ...
        <dl className="mt-4 grid grid-cols-2 gap-2">
          <div className="rounded-lg bg-neutral-50 px-2.5 py-2 text-center">
            <dt className="text-[10px] uppercase tracking-wide text-muted-foreground">Live now</dt>
            <dd className="mt-0.5 text-sm font-semibold tracking-tight">{liveCount}</dd>
          </div>
          <div className="rounded-lg bg-neutral-50 px-2.5 py-2 text-center">
            <dt className="text-[10px] uppercase tracking-wide text-muted-foreground">API</dt>
            <dd className="mt-0.5 text-sm font-semibold tracking-tight">Healthy</dd>
          </div>
        </dl>
      </section>
    </aside>
